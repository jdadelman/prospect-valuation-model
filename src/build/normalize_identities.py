...
@dataclass
class IdentityAgg:
    identity_key: str          # fgid if present, else no_fgid_*
    fgid: str                  # empty if missing
    player_name: str
    player_url: str
...
def read_reports_rows(path: Path) -> Iterable[dict[str, str]]:
    with path.open("r", newline="", encoding="utf-8") as f:
        r = csv.DictReader(f)
        required = {"rk", "fgid", "player_name", "player_url", "org_label", "report_year"}
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Directory containing reports_*.csv files",
    )
    ap.add_argument(
        "--out",
